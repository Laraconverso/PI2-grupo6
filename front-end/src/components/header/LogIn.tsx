...
const LogIn = ({ setModal }: { setModal: Dispatch<SetStateAction<"logIn" | "signUp" | "none">> }) => {
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen flex text-primary-50 justify-center">
      <div className="fixed inset-0 w-screen h-screen bg-black bg-opacity-20  backdrop-filter backdrop-blur-sm" onClick={closeModal} />

      <div className="relative flex flex-col bg-primary-500 bg-opacity-80 rounded-2xl text-center gap-7 z-10 my-auto py-4">
      <button className="font-semibold absolute top-3 right-5 cursor-pointer z-10" onClick={closeModal}>x</button>
        <p className="text-xl font-bauhs drop-shadow-md">INICIAR SESIÓN</p>
        <form action="" className="flex flex-col gap-4 w-screen sm:w-fit sm:p-4">
          <div className="py-2 px-4 bg-silver-50 rounded-2xl flex flex-col text-left text-silver-950 shadow-md gap-4">
            ...
            placeholder="DNI" />
            <input type="text" {...register("password")} className="bg-primary-200 p-1 focus-visible:outline-primary-700 focus-visible:bg-primary-100 rounded-md font-semibold text-center "
